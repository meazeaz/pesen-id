...
export default function ProductListClient({ initialProducts }: { initialProducts: Product[] }) {
  ...
  // Fitur Next.js/React terbaru untuk background proses anti-lag
  const [isPending, startTransition] = useTransition();

  // Logic Filter: Client Side Filtering (Cepat & Instan menggunakan data asli)
  ...
  const handleDelete = (id: string, title: string) => {
    // Tampilkan popup konfirmasi standar
    if (window.confirm(`Apakah Anda yakin ingin menghapus "${title}"?`)) {
      // Jalankan proses hapus di background tanpa menghentikan UI
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
           <div className="bg-white dark:bg-[#121212] border border-slate-200 dark:border-slate-800 p-5 rounded-2xl shadow-sm flex items-center gap-4 hover:border-purple-500/30 transition-colors group">
              ...
              <div>
                 ...
                 <h4 className="text-2xl font-bold mt-0.5">{initialProducts.length}</h4>
              </div>
           </div>
           <div className="bg-white dark:bg-[#121212] border border-slate-200 dark:border-slate-800 p-5 rounded-2xl shadow-sm flex items-center gap-4 hover:border-green-500/30 transition-colors group">
              ...
              <div>
                 ...
                 <h4 className="text-2xl font-bold mt-0.5">0</h4>
              </div>
           </div>
           <div className="bg-white dark:bg-[#121212] border border-slate-200 dark:border-slate-800 p-5 rounded-2xl shadow-sm flex items-center gap-4 hover:border-purple-500/30 transition-colors group">
                                ...
                                <div className="flex items-center gap-4">
                                   ...
                                   <div>
                                      ...
                                      <p className="text-[10px] text-slate-500 dark:text-slate-400 flex items-center gap-1 mt-1 font-mono">
                                         ID: #{product.id.slice(0, 6)} {/* Dipotong agar tidak terlalu panjang */}
                                      </p>
                                   </div>
                                </div>
                             </td>
                             ...
                             <td className="px-6 py-4 text-right">
                                <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                                   <Link 
                                       href={`/p/${product.id}`} 
                                       target="_blank"
                                       className="p-2 text-slate-400 hover:text-purple-600 hover:bg-purple-50 dark:hover:bg-purple-900/20 rounded-lg transition-colors inline-block" 
                                       title="Lihat Halaman Pembeli"
                                    >
                                       <Eye className="w-4 h-4" />
                                    </Link>
                                   <Link 
                                    href={`/dashboard/products/${product.id}`}
                                    className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors inline-block" 
                                    title="Edit"
                                    >
